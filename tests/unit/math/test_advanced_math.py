...
def test_optimize_jingcai_ticket():
    ...
    result = AdvancedLotteryMath.optimize_jingcai_ticket(
        num_legs=4, combined_odds=6000.0, target_investment=10000
    )
...
def test_calculate_zucai_value_index():
    ...
    matches = [{"true_prob": 0.5, "public_prob": 0.9}, {"true_prob": 0.4, "public_prob": 0.1}]
...
def test_calculate_last_leg_hedge():
    result = AdvancedLotteryMath.calculate_last_leg_hedge(
        original_bet=100, potential_payout=1000, hedge_odds={"Draw": 3.0, "Away": 4.0}
    )
