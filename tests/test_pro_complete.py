...
from src.calculations.pro.ml_predictors import (
    ModelTrainer,
    RandomForestPredictor,
    PredictionResult
)
from src.calculations.pro.advanced_backtest import AdvancedBacktestEngine
from src.calculations.pro.value_finder import ValueBetFinder, ValueBet
from src.calculations.pro.advanced_backtest import BetType
...
def main():
    ...
    if pred_poisson:
        ...
        print(f"   Expected Goals: {pred_poisson.home_goals_mean:.2f} - {pred_poisson.away_goals_mean:.2f}")
        print(f"   Probabilities: Home {pred_poisson.home_win_prob:.1%} | Draw {pred_poisson.draw_prob:.1%} | Away {pred_poisson.away_win_prob:.1%}")
    ...
    try:
        ...
        if pred_dc:
            ...
            print(f"   Probabilities: Home {pred_dc.home_win_prob:.1%} | Draw {pred_dc.draw_prob:.1%} | Away {pred_dc.away_win_prob:.1%}")
    ...
    try:
        ...
        print(f"   Probabilities: Home {rf_pred.home_win_prob:.1%} | Draw {rf_pred.draw_prob:.1%} | Away {rf_pred.away_win_prob:.1%}")
        print(f"   Confidence: {rf_pred.confidence:.1%}")
        
        if rf_pred.feature_importance:
            print(f"\n🏆 Top Features:")
            sorted_features = sorted(rf_pred.feature_importance.items(), key=lambda x: x[1], reverse=True)[:5]
