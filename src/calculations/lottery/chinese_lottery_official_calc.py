...
import numpy as np
from .lottery_knowledge import LOTTERY_KNOWLEDGE
...
class ChineseLotteryOfficialCalculator:
    ...
    官方规则来源: LOTTERY_KNOWLEDGE (单一真实来源)
    """
    jingcai_config = LOTTERY_KNOWLEDGE.get_lottery('JINGCAI')
    beidan_config = LOTTERY_KNOWLEDGE.get_lottery('BEIDAN')

    # 从单一真实来源读取所有规则，避免硬编码
    JINGCAI_RETURN_RATE = jingcai_config['return_rate']
    BEIDAN_RETURN_RATE = beidan_config['return_rate']
    JINGCAI_MAX_LEGS = jingcai_config['max_legs']
    BEIDAN_MAX_LEGS = beidan_config['max_legs']
    JINGCAI_BONUS_LIMITS = jingcai_config.get('bonus_limits', {})
    TAX_THRESHOLD = jingcai_config.get('tax_threshold', 10000.0)
    TAX_RATE = jingcai_config.get('tax_rate', 0.80)
    MINIMUM_BONUS = jingcai_config.get('minimum_bonus', 2.0)
    ...
    @staticmethod
    def calculate_jingcai_mxn_combinations(matches_count: int) -> dict:
        """
        竞彩 M串N 完整组合数计算
        例如 3串4 包含: 3个2串1 + 1个3串1 = 4注
        例如 4串11 包含: 6个2串1 + 4个3串1 + 1个4串1 = 11注
        返回每种过关方式的注数明细
        """
        if matches_count < 2:
            return {}
        ...
        combinations_result = {}
        for n in range(2, matches_count + 1):
            combinations_result[f"{n}串1"] = math.comb(matches_count, n)
        
        return combinations_result

    @staticmethod
    def calculate_jingcai_bonus(odds_list: list, bet_amount: float = 2.0, bet_count: int = 1) -> dict:
        """
        竞彩足球奖金计算
        官方公式: 2元 × 各场赔率连乘 × 倍数
        """
    ...
    def calculate_jingcai_max_bonus(selected_odds: list, pass_type: int, bet_amount: float = 2.0) -> float:
        ...
        # 2. 官方硬性单注封顶风控 - 从单一真实来源读取
        if pass_type <= 3:
            max_limit = ChineseLotteryOfficialCalculator.JINGCAI_BONUS_LIMITS.get('2_3_legs', 200000.0)
        elif pass_type <= 5:
            max_limit = ChineseLotteryOfficialCalculator.JINGCAI_BONUS_LIMITS.get('4_5_legs', 500000.0)
        else:
            max_limit = ChineseLotteryOfficialCalculator.JINGCAI_BONUS_LIMITS.get('6_plus_legs', 1000000.0)
        ...
        for combo in combos:
            ...
            # 应用国家个人所得税
            if combo_bonus >= ChineseLotteryOfficialCalculator.TAX_THRESHOLD:
                combo_bonus *= ChineseLotteryOfficialCalculator.TAX_RATE
    ...
    def estimate_toto14_prize(total_pool: float, rollover_pool: float, estimated_winners_1st: int, estimated_winners_2nd: int) -> dict:
        ...
        # 依法扣税
        if prize_1st_single >= ChineseLotteryOfficialCalculator.TAX_THRESHOLD:
            prize_1st_single *= ChineseLotteryOfficialCalculator.TAX_RATE
        if prize_2nd_single >= ChineseLotteryOfficialCalculator.TAX_THRESHOLD:
            prize_2nd_single *= ChineseLotteryOfficialCalculator.TAX_RATE
