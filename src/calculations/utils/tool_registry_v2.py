...
from src.tools.intel.intelligence_gatherer import gather_match_intelligence
from src.tools.execution.simulated_ticket import generate_simulated_ticket
from src.tools.odds.market_deep_analyzer import deep_evaluate_all_markets
from src.tools.odds.global_odds_fetcher import get_global_arbitrage_data
from src.calculations.quant.trap_identifier import identify_low_odds_trap
from src.calculations.quant.latency_arbitrage import detect_latency_arbitrage
from src.calculations.odds.betfair_anomaly import detect_betfair_anomaly
from src.calculations.quant.kelly_variance_analyzer import analyze_kelly_variance
import torch

def _run_stgnn_simulation(home_team: str, away_team: str, current_minute: int = 70) -> dict:
    """包装 ST-GNN 模拟器的执行入口"""
    from src.calculations.quant.st_gnn_simulator import GenerativeWorldModel, fetch_real_tracking_data
    try:
        # 1. 初始化模型
...
def _data_gateway_fetch_match(date: str, lottery_type: str = "JINGCAI"):
    ...
    try:
        from src.core.data_gateway import DataGateway
        gw = DataGateway()
...
def _data_gateway_fetch_odds(league: str, home_team: str, away_team: str):
    ...
    try:
        from src.core.data_gateway import DataGateway
        from src.tools.odds.entity_resolver import resolver
...
def _fetch_arbitrage_news(team_name: str):
    """获取毫秒级突发套利新闻"""
    try:
        from src.tools.intel.social_news_listener import SocialNewsListener
        listener = SocialNewsListener(use_mock=False)
        return listener.fetch_latest_news(team_name)
    except ImportError:
        return {"ok": False, "error": "social_news_listener 模块不可用"}
...
def _calc_complex_parlay(**kw):
    from src.tools.execution.atomic_skills import calculate_jingcai_parlay_prize
    return calculate_jingcai_parlay_prize(kw["matches_odds"], kw["m"], kw["n"])


def _calc_chuantong(**kw):
    from src.tools.execution.parlay_rules_engine import ParlayRulesEngine
